        ...
        for f in files:
            if f.lower().endswith(".pdf"):
                try:
                    ...
                    continue
    return round(total_size / (1024 * 1024), 2), file_count  # MB
...
def get_recent_files(folder, limit=10):
    files = []
        ...
        for fname in filenames:
            if fname.lower().endswith(".pdf"):
                ...
                try:
                    ...
                    files.append((mtime, fname, round(size_mb, 2)))
                except:
                    continue
...
def choose_folder(label, frame):
    ...
    if new_folder:
        ...
        LOG_PATH = os.path.join(WATCH_FOLDER, "patent_ob_batchs_log.csv")
        label.config(text=f"📁 Monitoring: {WATCH_FOLDER}")
        update_gui(frame, label)


def download_logo_if_needed():
    logo_path = os.path.abspath("../asset/nct_logo_3000x3000_20250606.png")
...
def start_gui():
    global tree
    root = tk.Tk()
    root.title("📈 Patent Batch Monitor GUI")
    ...
    for col in columns:
        anchor = tk.W if col == "File Name" else tk.CENTER  # 👈 Căn trái cho "File Name"
    ...
    footer_label = tk.Label(bottom_frame, text=footer_text, font=("Arial", 9), justify="center")
    footer_label.pack()
...
def main():
    ...
    parser.add_argument("-i", "--input", type=str, help="Input folder to monitor", required=False)
    ...
    if args.input:
        ...
        WATCH_FOLDER = os.path.abspath("../database/orange_book/patent_ob_batchs")
...
# how to run:
# python Monitor_Folder_Gui.py

# python Monitor_Folder_Gui.py -i "D:\Pharmacopoeia"
